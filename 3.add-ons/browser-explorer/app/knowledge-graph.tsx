...
export default function KnowledgeGraph({ graph, loading = false }: { graph: GraphData; loading?: boolean }) {
  ...
  useEffect(() => {
    ...
    const closeOnDesktop = () => { if (!mobile.matches) mobileDialogRef.current?.close(); };
  ...
  const nodeDetails = selected ? <><div className="detail-kind"><span />{selected.kind}</div><h3>{selected.title}</h3><p>{selected.excerpt || "No summary is available for this record."}</p>{Object.keys(selected.details).length > 0 && <dl className="detail-metadata">{selected.details.effectiveDate && <div><dt>Effective</dt><dd>{readableDate(selected.details.effectiveDate)}</dd></div>}{selected.details.lastConfirmedDate && <div><dt>Last confirmed</dt><dd>{readableDate(selected.details.lastConfirmedDate)}</dd></div>}{selected.details.source && <div><dt>Source</dt><dd>{selected.details.source}</dd></div>}</dl>}<dl className="detail-stats"><div><dt>Collection</dt><dd>{selected.collection}</dd></div><div><dt>Connections</dt><dd>{neighbours.size}</dd></div><div><dt>Current state</dt><dd>{selected.stateCount}</dd></div><div><dt>Events</dt><dd>{selected.eventCount}</dd></div></dl><div className="detail-themes"><span>Linked Core themes</span>{selected.themeIds.length ? selected.themeIds.map((id) => <small key={id}><i style={{ background: themeColours.get(id) }} />{themeNames.get(id)}</small>) : <p>None. Colours appear only for reciprocal Markdown links.</p>}</div>{selectedSections.length > 0 && <div className="detail-sections"><span>Sections</span>{selectedSections.map(({ heading, id }) => <a key={id} href={recordHref(selected.path, id)}>{heading}</a>)}</div>}<div className="detail-files"><span>Associated Markdown files</span>{associatedFiles.length ? associatedFiles.map((file) => <a key={file.path} href={recordHref(file.path)} target="_blank" rel="noreferrer">{file.title}<span>View <i className="fa-solid fa-arrow-up-right-from-square" aria-hidden="true" /></span></a>) : <p>No Markdown files are associated with this node.</p>}</div></> : null;
  ...
  return <section className={`graph-workspace ${detailOpen ? "" : "detail-collapsed"}`}>
    <div className="graph-stage-wrap"><div className="graph-stage">
      ...
      <details className="graph-legend" ref={initializeLegend}>
        ...
        <div className="graph-legend-content"><span><i className="legend-record" />Record · fill shows linked theme</span><span><i className="legend-theme" />Theme</span><span><i className="legend-collection" />Collection</span><span><i className="legend-reference" />Explicit Markdown link</span>{graph.themes.map((item) => <span key={item.id}><i style={{ background: themeColours.get(item.id) }} />{item.title}</span>)}</div>
      </details>
    </div>
    <aside id="graph-node-details" aria-label="Node details" className="graph-detail" aria-live="polite"><div className="detail-panel-header"><button className="panel-toggle detail-toggle" type="button" onClick={() => setDetailOpen((open) => !open)} aria-label={detailOpen ? "Collapse node information panel" : "Expand node information panel"} aria-expanded={detailOpen}><i className={`fa-solid ${detailOpen ? "fa-chevron-right" : "fa-chevron-left"}`} aria-hidden="true" /></button><span>{detailOpen ? "Hide details" : "Show details"}</span></div><div className="detail-content">{selected ? <><button className={localOnly ? "active" : ""} onClick={() => setLocalOnly((value) => !value)}>{localOnly ? "Show full graph" : "Focus on neighbours"}</button>{nodeDetails}</> : <div className="detail-empty"><span><i className="fa-solid fa-arrow-pointer" aria-hidden="true" /></span><h3>Select a node</h3><p>Click or press Enter to inspect it. Drag to arrange; arrow keys move a focused node.</p></div>}</div></aside></div>
    <dialog ref={mobileDialogRef} id="mobile-node-information" className="mobile-node-dialog" aria-label="Node information">
      <div className="mobile-dialog-header"><span>Node information</span><button type="button" onClick={() => mobileDialogRef.current?.close()} aria-label="Close node information" autoFocus><i className="fa-solid fa-xmark" aria-hidden="true" /></button></div>
