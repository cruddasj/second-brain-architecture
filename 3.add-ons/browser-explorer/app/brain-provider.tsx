...
type SavedBrainState = {
  snapshot: Snapshot | null;
  connection: Connection | null;
};

type BrainContextValue = SavedBrainState & {
  data: BrainData;
  loading: boolean;
  busy: boolean;
  message: string;
...
export function useBrain() {
  ...
  if (!value) throw new Error("Missing reader provider");
  return value;
}

export default function BrainProvider({ children }: { children: ReactNode }) {
  const [state, setState] = useState<SavedBrainState>({ snapshot: null, connection: null });
  const [data, setData] = useState<BrainData>(emptyBrainData);
    ...
    async function restore() {
      try {
        if (localMode) {
          ...
          if (active) {
            setData(local);
            setState({ snapshot: local.snapshot, connection: null });
          }
        } else {
          const saved = await readState() as SavedBrainState;
          const indexed = saved.snapshot ? buildBrainData(saved.snapshot.files) as BrainData : emptyBrainData;
          if (active) {
            setState(saved);
            setData(indexed);
          }
  ...
  async function sync(repository: string, token: string, remember: boolean) {
    ...
    try {
      ...
      setMessage(error instanceof TypeError ? "Could not reach the repository. Your saved content is unchanged." : (error as Error).message);
    } finally {
      lock.current = false;
      setBusy(false);
    }
  ...
  async function disconnect() {
    ...
    try {
      ...
      try {
        localStorage.removeItem(`second-brain-graph-positions-v1:${storageScope}`);
      } catch {
        /* Layout storage may be independently disabled. */
      }
      setState({ snapshot: null, connection: null });
      setData(emptyBrainData);
      setMessage("Connection and downloaded content removed from this browser.");
    } catch (error) {
